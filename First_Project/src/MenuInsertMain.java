...
public class MenuInsertMain extends JFrame implements ActionListener {
	...
	public MenuInsertMain(String id, String storeName) {
		this.id = id;
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (obj == btn[0]) {
			...
			if (check) {
				new SellerMenuMain(id, storeName);
				dispose();
			}
		}
	...
	private boolean insertData() {
		...
		if (check) {
			MenuBean meb = new MenuBean(id, storeName, txt[0].getText(), Integer.parseInt(txt[1].getText()),
					Integer.parseInt(txt[2].getText()), txt[3].getText(), txt[4].getText());

			int cnt = dao.insertData(meb);
			if (cnt == -1)
