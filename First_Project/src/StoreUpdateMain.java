...
public class StoreUpdateMain extends JFrame implements ActionListener {
	...
	public StoreUpdateMain(StoreBean sb) {
		this.sb = sb;
		this.storeName = sb.getStoreName();
		setTitle("가게등록");
	...
	private void textCopy() {
		txt[0].setText(sb.getStoreName());
		txt[1].setText(sb.getFoodType());
		txt[2].setText(String.valueOf(sb.getPriceAvg()));
		txt[3].setText(sb.getStoreLocation());
		txt[4].setText(String.valueOf(sb.getCallNum()));
		txt[5].setText(String.valueOf(sb.getReviewCnt()));
	}
	...
	private boolean updateData() {
		...
		if (check) {
			sb = new StoreBean(sb.getId(), txt[0].getText(), txt[1].getText(), Integer.parseInt(txt[2].getText()),
					txt[3].getText(), txt[4].getText(), Integer.parseInt(txt[5].getText()));

			int cnt = dao.updateData(sb, storeName);
			if (cnt == -1)
